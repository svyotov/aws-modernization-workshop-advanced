...
----
+
Step 7:: After [underline]#40 - 60# seconds, re-run the `kubectl describe` command to view the `Events` output again and see what atctions the `kubelet` took.
+
Example Output:
+
[.output]
...
+
NOTE: Ensure that you are working in this modules directory. i.e. `~/environment/aws-modernization-workshop-advanced/modules/monitor-application`
+
[source,shell]
----
...
+
Step 9:: In the *Filter:* box, enter `eks` and press `[ENTER]` to filter the Log Group for our EKS cluster. Click on the `/eks/petstore/containers` Log Group.
+
