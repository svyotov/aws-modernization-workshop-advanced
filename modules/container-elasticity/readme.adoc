...
....
+
Step 10:: Now we should again log the `cluster-autoscaler` pod and you will see it has update the `desired` count of instances to reflect that.
+
[source,shell]
----
kubectl logs -f deploy/cluster-autoscaler --namespace kube-system -f
...
....
+
Step 12:: Now that we have the instance cordoned from `down scaling` we can then `scale` the `--replicas` to `2` by issueing the folling commanbd:
+
[source,shell]
----
